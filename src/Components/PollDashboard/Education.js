...
import logo from '../assets/LogoSahfiya.jpeg'

const Education = () => {
  const [polls, setPolls] = useState([]);
  const [responses, setResponses] = useState({});
  const [currentPage, setCurrentPage] = useState(0); // To track the current page of polls
  const pollsPerPage = 3; // Number of polls to display per page
  const navigate = useNavigate();

  useEffect(() => {
    const fetchPolls = async () => {
      try {
        const response = await axios.get('http://localhost:8080/api/polls/category/Education');
  ...
  return (
    <div className="polls-wrapper">
      <div className="education-poll-container">
        ...
        <button className="back-buttons" onClick={() => navigate('/publicdashboard')}>Home</button>
      </div>
      {polls.length === 0 ? (
        <>
            ...
            {displayedPolls.map((pollDetails) => {
              ...
              return (
                <div key={pollDetails.pollId} className="poll-card" style={style}>
                  ...
                  <button className="submit" style={{ width: "300px" }} onClick={() => handleSubmit(pollDetails.pollId)}>Submit</button>
                </div>
              );
            })}
          </div>
